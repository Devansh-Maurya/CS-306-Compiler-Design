...
void applyNonPreemptivePriorityScheduling(Process *, Process *, int);
int arrivalTimeComparator(const void*, const void*);
int priorityComparator(const void *, const void *);
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        printf("Enter batch time for process %d: ", i+1);
        scanf("%d", &processes[i].batchTime);
    ...
    qsort((void *) processes, k + 1, sizeof(Process), priorityComparator);

    applyNonPreemptivePriorityScheduling(processes, ganttChart, n);
...
void applyNonPreemptivePriorityScheduling(Process *processes, Process *ganttChart, int n) {

    processes[0].completionTime = processes[0].arrivalTime + processes[0].batchTime;
    processes[0].turnaroundTime = processes[0].batchTime;
    ...
    for (j = 1; (j < n) && (arrivedProcessesCount < n-1); j++) {
        ...
        qsort(processPtr, arrivedProcessesCount-1, sizeof(Process), priorityComparator);

        processes[j].completionTime = processes[j-1].completionTime + processes[j].batchTime;
        processes[j].turnaroundTime = processes[j].completionTime - processes[j].arrivalTime;
        processes[j].waitingTime = processes[j].turnaroundTime - processes[j].batchTime;
    ...
    for (int k = j; k < n; ++k) {
        processes[k].completionTime = processes[k-1].completionTime + processes[k].batchTime;
        processes[k].turnaroundTime = processes[k].completionTime - processes[k].arrivalTime;
        processes[k].waitingTime = processes[k].turnaroundTime - processes[k].batchTime;
...
int priorityComparator(const void *p, const void *q) {

    int a = (*(Process *) p).priority;
    int b = (*(Process *) q).priority;
...
void printProcessArray(const Process* processes, int n) {
    ...
    printf("\nProcess\tArrival time\t\tBatch time\t\tPriority\t\tCompletion time\t\tTurnaround time\t\tWaiting time\n");
    for (int i = 0; i < n; ++i) {
        printf("%d\t\t\t\t%d\t\t\t\t%d\t\t\t\t%d\t\t\t%d\t\t\t\t%d\t\t\t\t%d\n",
               processes[i].pId, processes[i].arrivalTime, processes[i].batchTime,
